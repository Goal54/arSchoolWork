...
open class ARViewController: UIViewController, ARTrackingManagerDelegate
{
    ...
    internal func initializeInternal()
    {
        ...
        // Default values
      self.trackingManager.delegate = self as! ARTrackingManagerDelegate
  ...
  @objc internal func displayTimerTick()
    ...
    fileprivate func loadCamera()
    {
        ...
        //===== View preview layer
        if cameraLayer == AVCaptureVideoPreviewLayer(session: self.cameraSession)
        {
    ...
    open class func createCaptureSession() -> (session: AVCaptureSession?, error: NSError?)
    {
      ...
      backVideoDevice = AVCaptureDevice.default(.builtInWideAngleCamera, for: AVMediaType.video, position: .back)
